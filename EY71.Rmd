...
for (i in c(1:nrow(df1))) {
  ...
  fly <- df1$Fly[i]
  pos <- df1$Position[i] 
  brightness <- df1$Brightness[i]
  max_value <- filter(newdf1, ID == lab & Fly == fly) %>%
...
```{r}
sp <- ggplot(h, aes(x = Plate, y = glow, color= Time))+ 
  geom_jitter(width=0.2) +  xlab("Plate Type") + ylab("Percent glowing")
...
```{r}
sp <- ggplot(g, aes(x = T2, y = Lum/CFU, color= Color))+ 
  geom_point(size=2.5) +  xlab("Hour") + ylab("Lum/CFU")
